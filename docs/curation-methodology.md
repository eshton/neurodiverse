...
## Articles (journalism, not content-marketing)

The `articles` category is written journalism/explainers about ADHD/autism from reputable Hungarian outlets. Rules that emerged seeding it (2026-07-05):

- **Journalism, not SEO.** Include real editorial pieces (Telex, HVG360, Qubit, WMN, Képmás, and similar — 444, G7, Szabad Európa, Forbes.hu are good untapped ones). **Exclude clinic/webshop content-marketing** (adhdspecialista.hu, mindsetpszichologia.hu, netamin, etc.) — they can be informative but they're ads/SEO with a conflict of interest, and a directory that mixes them in loses trust. Also exclude churnalism/tabloid science (origo.hu's "ADHD changes your gait", borsonline, egeszsegkalauz).
- **Verify title + date even when WebFetch 403s.** WMN and hvg.hu 403'd WebFetch, but plain `curl -sL -A "Mozilla/5.0…"` got through and the metadata is in the page head: `og:title`, `article:published_time` (HVG), and JSON-LD `"datePublished"`/`"author"` (Telex, HVG). Grep those. Telex's byline isn't in the OG/JSON-LD reliably, so `author` was often omitted (it's optional). When a date genuinely can't be extracted (both WMN pieces), **omit `publishedDate` rather than guess** — and say so in the body.
- **Paywalls are OK but flag them.** hvg360 articles are real and linkable but behind a subscription wall (curl still gets the OG metadata). Include them, but note "előfizetéses hvg360-tartalom" in the body so the reader isn't surprised.
- **Screen for the same honesty bar as everything else** — no anti-vax/"cure autism" opinion pieces, however well-written.
...
- **Reputable HU sources found** (all fetch-verified, expert-authored): koraifejleszto.hu (Korai Fejlesztő Központ / Autizmus Alapítvány), kiskanalkommando.hu (gyermekdietetikus — has good ARFID content too), aosz.hu, szafi-dietetikai-team.hu (dietitian team), pharmaonline.hu (multiple ADHD/nutrient articles: omega-3, D-vitamin, and "Természetes megoldások az ADHD kezelésére" which covers iron+zinc), 21napalatt.hu (recipes). These are good return-to sources for a second pass (covers, more recipes).
- **Verify which nutrient a source *actually* covers before writing the entry.** A WebSearch result summary claimed a Pharma Online article discussed magnesium+B6 for ADHD; fetching the article showed it only covers iron+zinc — the summary had conflated multiple search hits. The iron+zinc entry was written to the article's real content (deficiency-conditional, complementary not curative). Don't publish a nutrient claim you haven't confirmed on the actual page.
- **Don't force an entry when only supplement-shop sources exist.** A standalone iron entry with the (real, valuable) "routine iron supplementation isn't recommended for ADHD, only correct documented deficiency / RLS with ferritin <50" nuance was *skipped* because that nuance only surfaced on supplement shops (netamin, anatur) and functional-medicine sites — no reputable clinical page stated it cleanly. Better an honest gap than a shop-sourced entry.
- **Some "picky-eater recipe" angles fight the feeding-therapy guidance.** "Hidden/sneaked vegetables" (csempészett zöldség) is popular in HU parenting blogs but conflicts with the exposure/food-chaining, no-pressure approach the curated feeding guides (koraifejleszto, kiskanalkommando) endorse — feeding therapists often warn that sneaking breaks mealtime trust. Kept the category internally consistent by not adding it.
...
**Articles & research covers (2026-07-05).** Scraping `og:image` works well for `articles` — every outlet (Telex, HVG, WMN, Qubit, Képmás, Szabad Európa) sets a per-article hero image, so all 21 got a real, distinct cover via `curl` + `og:image` (add a `Referer` header for CDN downloads, and `file -b --mime-type` the bytes — Telex serves WebP under `.jpg`-looking URLs). `research` is the opposite: most **DOI/journal/registry pages expose no `og:image`** (or only a publisher logo), so only 3/15 landed — and those are actual figures from the papers. **Verify a scraped research figure actually belongs to its paper**: Simon 2009's scrape returned Modabbernia's forest plot (caught because the two files were byte-identical) — a topic mismatch (an ADHD prevalence paper showing an autism environmental-risk plot) is the tell. When in doubt, drop it rather than show a wrong/generic image. **For research papers with no `og:image`, go through PubMed Central**: resolve the DOI → PMCID with NCBI's `idconv` API (`https://www.ncbi.nlm.nih.gov/pmc/utils/idconv/v1.0/?ids=<doi>&format=json`), fetch `https://pmc.ncbi.nlm.nih.gov/articles/PMC<id>/`, and grep the HTML for figure blobs (`cdn.ncbi.nlm.nih.gov/pmc/blobs/.../grN.jpg` or `.../…fN.jpg`). Prefer a legible *results* figure (network/forest/bar plot) over a PRISMA flow diagram or study-design schematic. Paywalled papers with no PMC version (many Lancet/Elsevier seminars) and registry pages (ClinicalTrials.gov) simply have no figure to take — leave them cover-less.
...
Tags render verbatim as `#tag` chips on the detail page and the slide-over panel — they are **display-only** (no routing, filtering, or slugs depend on them), so their sole job is to read well to a Hungarian user. Convention, enforced by a one-time normalization pass over the whole corpus:

- **Hungarian, lowercase, hyphenated, with correct accents** — `korai-fejlesztés`, `állami-finanszírozás`, `beutaló-nélkül`, `szülők`. No spaces (`nagy minta` → `nagy-minta`), no accent-stripping (`diagnozis` → `diagnózis`).
- **One canonical tag per concept** — don't reintroduce synonyms that collapse to the same idea: use `felnőttek` (not `adults`), `nők` (not `women`/`no`), `állami-finanszírozás` (not `state`/`state-funded`), `interjú` (not `interview`/`interju`/`interviews`), `oktatás` (not `education`/`educational`).
- **English stays only for genuine proper-noun method/acronym names** with no Hungarian equivalent: `aba`, `teacch`, `tsmt`, `aac`, `spd`, `arfid`, `audhd`, `snoezelen`, `food-chaining`, plus adopted loanwords already standard in Hungarian (`coaching`, `online`, `fidget`). Everything else is translated.
- Before adding a tag, check it isn't a near-duplicate of one already in `public/db/content.json` (`.items[].tags`). The normalizer script (one-off) lived at `scratchpad/normalize-tags.mjs` if a bulk re-pass is ever needed.
